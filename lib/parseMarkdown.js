 ...
 *  - iframe (유튜브 영상 삽입 등, src/width/height/allow류 속성만 허용)
...
export function sanitizeHtml(html) {
  ...
  // DOMPurify 없을 때 — script/이벤트핸들러/javascript: 만 제거.
  // iframe(유튜브 영상 삽입 등)은 관리자가 직접 붙여넣는 신뢰된 콘텐츠라 그대로 통과시킨다
  // (svg/table/img 등 다른 raw HTML 태그도 이 경로에서 이미 그대로 통과되고 있음, iframe만 별도로 막을 이유 없음).
  return html
...
export function parseMarkdown(md) {
  ...
  // 1-1) iframe 블록 분리 보존 (유튜브 영상 삽입 등, 변환 중 깨지지 않게)
