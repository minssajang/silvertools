...
const TOOL_OPTIONS = [
  { value: 'magnifier-down', label: '돋보기' },
  { value: 'medicine',       label: '복약관리' },
  { value: 'hospital',       label: '병원찾기' },
  { value: 'sos',            label: '긴급SOS' },
  { value: 'brain-game',     label: '두뇌게임' },
  { value: 'health-record',  label: '건강기록' },
  { value: 'big-news',       label: '큰글씨뉴스' },
...
function TabManagerModal({ tabs, onClose, onSave }) {
  ...
  const addTab = () => {
    ...
    setLocalTabs(prev => [...prev, { id, label, icon: '📌' }])
    setNewLabel('')
  }
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 9000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 14, padding: 28, width: 460, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {form.type === 'keyword' && (
            <div>
              <label style={S.label}>검색 키워드 (정확한 키워드명)</label>
              <input value={form.keyword} onChange={e => set('keyword', e.target.value)} style={{ ...S.input }} placeholder="예: 노인 낙상 예방" />
            </div>
          )}
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 20, justifyContent: 'flex-end' }}>
          <button onClick={onClose} style={{ ...S.btnGhost }}>취소</button>
          <button onClick={submit} disabled={!form.content.trim()} style={{ ...S.btn(), opacity: form.content.trim() ? 1 : 0.4 }}>저장</button>
        </div>
      </div>
    </div>
  )
...
export default function ContentIdeaPanel({ adminToken }) {
  ...
  return (
    <div>
      ...
      {/* 탭 바 */}
      <div style={{ display: 'flex', gap: 4, marginBottom: 20, flexWrap: 'wrap', borderBottom: '1px solid #2a2a2a', paddingBottom: 0 }}>
        {tabs.map(tab => {
          ...
          return (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{
              ...
              color: activeTab === tab.id ? '#e63946' : '#888',
              ...
              <span>{tab.label}</span>
              {cnt > 0 && (
                <span style={{ fontSize: 11, background: '#e6394622', color: '#e63946', padding: '1px 6px', borderRadius: 10, fontWeight: 700 }}>
                  {cnt}
                </span>
              )}
            </button>
            ...
            <div key={idea.id} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {idea.keyword && (
                    <div style={{ fontSize: 12, color: '#888', marginTop: 4 }}>🔑 {idea.keyword}</div>
                  )}
                  {idea.memo && (
                    <div style={{ fontSize: 12, color: '#666', marginTop: 4, fontStyle: 'italic' }}>📝 {idea.memo}</div>
                  )}
                </div>
                <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                  <button onClick={() => toggleStatus(idea.id, idea.status)} title={idea.status === 'used' ? '미사용으로' : '사용 처리'}
