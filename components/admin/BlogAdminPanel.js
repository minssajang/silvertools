...
export default function BlogAdminPanel({ adminToken, initialView }) {
  ...
  const handleEdit = (post) => {
    ...
    setForm({
      ...
      category: post.category || 'magnifier-down',
  ...
  const filteredPosts = filterType === 'all' ? posts : posts.filter(p => (p.category||'magnifier-down') === filterType)
            ...
            <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginBottom:16 }}>
              {[['all','전체'], ...allCategories.map(c=>[c,c])].map(([key,label]) => (
                <button key={key} onClick={()=>setFilterType(key)}
                  ...
                  {label} {key==='all'?posts.length:posts.filter(p=>(p.category||'magnifier-down')===key).length}
                </button>
              ))}
            </div>
