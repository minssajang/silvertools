...
export default function Login() {
  ...
  useEffect(() => {
    ...
    // 이미 로그인 상태면 홈으로
    const user = localStorage.getItem('silver_user')
    if (user) router.push('/')
  ...
  const handleSubmit = async () => {
    ...
    if (mode === 'register' && password.length < 4) { setError(lang === 'ko' ? '비밀번호는 4자리 이상' : 'Password min 4 chars'); return }
    ...
    try {
      ...
      localStorage.setItem('silver_user', JSON.stringify(data.user))
      router.push('/')
    } catch {
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .mode-tabs { display: flex; gap: 0; margin-bottom: 24px; border-radius: 12px; overflow: hidden; border: 2px solid var(--border); }
        .mode-tab { flex: 1; padding: 12px; border: none; background: var(--surface2); color: var(--text2); font-size: 16px; font-weight: 700; cursor: pointer; transition: all .2s; }
        ...
        .form-input { width: 100%; padding: 16px 18px; border-radius: 12px; border: 2px solid var(--border); background: var(--surface2); color: var(--text); font-size: 18px; outline: none; transition: border-color .2s; }
        .form-input:focus { border-color: ${ACCENT}; }
        .submit-btn { width: 100%; padding: 18px; border-radius: 14px; background: ${ACCENT}; color: #fff; border: none; font-size: 20px; font-weight: 900; cursor: pointer; margin-top: 8px; transition: all .15s; }
      ...
      <main className="login-wrap">
        <div className="login-card">
          ...
          <div className="form-group">
            <label className="form-label">{lang === 'ko' ? '전화번호' : 'Phone Number'}</label>
          ...
          <div className="form-group">
            <label className="form-label">{lang === 'ko' ? '비밀번호' : 'Password'}</label>
            <input className="form-input" type="password" value={password}
              onChange={e => setPassword(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && handleSubmit()}
              placeholder={lang === 'ko' ? '숫자 4자리 이상' : 'Min 4 characters'} />
          </div>
          ...
          {mode === 'register' && (
            <div className="invite-info">
              💡 {lang === 'ko'
                ? '가입하면 6자리 초대코드가 생성됩니다. 자녀에게 코드를 알려주면 복약·건강 기록을 함께 볼 수 있어요.'
                : 'After registering, you\'ll get a 6-digit invite code to share with family members.'}
            </div>
          )}
        </div>
      </main>
