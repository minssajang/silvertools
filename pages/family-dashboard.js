...
export default function FamilyDashboard() {
  ...
  const [tab, setTab] = useState(0)  // 0:복약 1:건강 2:SOS
  ...
  const loadLinkedSeniors = async (family_id) => {
    const res = await fetch('/api/silver/auth', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'get_linked_seniors', family_id })
    })
  ...
  const loadMedData = async (senior_id) => {
    setDataLoading(true)
    const res = await fetch(`/api/silver/medicine?user_id=${senior_id}&view_as=${senior_id}`)
    const data = await res.json()
    setMedData(data)
  ...
  const loadHealthData = async (senior_id, type) => {
    setDataLoading(true)
    const res = await fetch(`/api/silver/health?user_id=${senior_id}&view_as=${senior_id}&type=${type}`)
    const data = await res.json()
  ...
  const loadSosData = async (senior_id) => {
    setDataLoading(true)
    const res = await fetch(`/api/silver/health?user_id=${senior_id}&view_as=${senior_id}`)
    const data = await res.json()
    setSosData(data)
    setDataLoading(false)
  }
  ...
  const handleLink = async () => {
    ...
    const res = await fetch('/api/silver/auth', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'link_family', family_id: user.id, invite_code: inviteInput.trim() })
    })
    const data = await res.json()
    if (!res.ok) { setLinkError(data.error); setLinkLoading(false); return }
    setInviteInput('')
    loadLinkedSeniors(user.id)
  ...
  const HEALTH_TYPES = [
    { key: 'bp', ko: '혈압', en: 'Blood Pressure', icon: '❤️', unit: 'mmHg' },
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .senior-chip { padding: 10px 18px; border-radius: 99px; border: 2px solid var(--border); background: var(--surface); color: var(--text); font-size: 15px; font-weight: 700; cursor: pointer; transition: all .2s; }
        ...
        .invite-input { flex: 1; padding: 12px 16px; border-radius: 10px; border: 2px solid var(--border); background: var(--surface2); color: var(--text); font-size: 16px; font-weight: 700; letter-spacing: 4px; outline: none; text-transform: uppercase; }
        .invite-input:focus { border-color: ${ACCENT}; }
        .invite-btn { padding: 12px 20px; border-radius: 10px; background: ${ACCENT}; color: #fff; border: none; font-size: 15px; font-weight: 700; cursor: pointer; white-space: nowrap; }
        .dash-tabs { display: flex; gap: 8px; margin-bottom: 16px; }
        .dash-tab { flex: 1; padding: 12px 6px; border-radius: 12px; border: 2px solid transparent; background: var(--surface); color: var(--text); font-size: 15px; font-weight: 700; cursor: pointer; text-align: center; transition: all .2s; }
        ...
        .med-header { display: flex; align-items: center; gap: 10px; margin-bottom: 8px; }
        .med-dot { width: 12px; height: 12px; border-radius: 50%; }
        .med-name { font-size: 18px; font-weight: 800; }
        ...
        .rec-val { font-size: 20px; font-weight: 900; flex: 1; }
      ...
      <main className="dash-wrap">
        ...
        <button className="screen-share-btn" onClick={() => router.push('/screen-view')}>
          📱 {lang === 'ko' ? '부모님 화면 보기 (화면공유)' : 'View Parent Screen (Share)'}
        </button>

        {/* 초대코드 입력 */}
        <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text2)', marginBottom: 8 }}>
          🔗 {lang === 'ko' ? '초대코드로 부모님 연동' : 'Link Parent with Invite Code'}
        </div>
        ...
        {seniors.length > 0 && (
          <div className="senior-selector">
            <span style={{ fontSize: 14, color: 'var(--text2)', fontWeight: 700 }}>👤</span>
            {seniors.map(s => (
          ...
          <>
            ...
            {tab === 0 && totalDoses > 0 && takenDoses < totalDoses && (
              <div className="alert-box">
                ⚠️ {lang === 'ko'
                  ? `${selectedSenior.name || '부모님'}께서 오늘 ${totalDoses - takenDoses}회 복약을 아직 안 하셨습니다`
                  : `${selectedSenior.name} has ${totalDoses - takenDoses} missed dose(s) today`}
              </div>
            )}
              ...
              <>
                <div className="stat-row">
                  <div className="stat-box">
                    <div className="stat-num">{totalDoses}</div>
                    <div className="stat-label">{lang === 'ko' ? '오늘 복용 예정' : 'Scheduled'}</div>
                  </div>
                  <div className="stat-box">
                    <div className="stat-num" style={{ color: '#10b981' }}>{takenDoses}</div>
                    <div className="stat-label">{lang === 'ko' ? '복용 완료' : 'Taken'}</div>
                  </div>
                  <div className="stat-box">
                    <div className="stat-num" style={{ color: '#ef4444' }}>{totalDoses - takenDoses}</div>
                    <div className="stat-label">{lang === 'ko' ? '미복용' : 'Missed'}</div>
                  </div>
                </div>
                {todayMeds.length === 0 ? (
            ...
            {tab === 1 && (
              <>
                <div className="health-type-row">
                  {HEALTH_TYPES.map(ht => (
                    <button key={ht.key} className={`health-type-btn${healthType === ht.key ? ' active' : ''}`}
                      onClick={() => setHealthType(ht.key)}>
                      <div style={{ fontSize: 18 }}>{ht.icon}</div>
                      <div>{lang === 'ko' ? ht.ko : ht.en}</div>
                    </button>
                  ))}
                </div>
                {healthData.length === 0 ? (
                  ...
                  <div key={rec.id} className="rec-row">
                    <div className="rec-val" style={{ color: HEALTH_TYPES.find(h => h.key === healthType)?.color || ACCENT }}>
                      {rec.v1}{rec.v2 ? `/${rec.v2}` : ''} <span style={{ fontSize: 13, color: 'var(--text2)', fontWeight: 400 }}>{HEALTH_TYPES.find(h => h.key === healthType)?.unit}</span>
                    </div>
            ...
            {tab === 2 && (
              <>
                <div style={{ fontSize: 16, fontWeight: 800, marginBottom: 12 }}>📞 {lang === 'ko' ? '긴급 연락처' : 'Emergency Contacts'}</div>
                ...
                {sosData.contacts.length === 0 && (
                  <div className="empty"><div style={{ fontSize: 40 }}>📞</div><div style={{ marginTop: 10 }}>{lang === 'ko' ? '등록된 연락처 없음' : 'No contacts'}</div></div>
                )}

                <div style={{ fontSize: 16, fontWeight: 800, margin: '20px 0 12px' }}>🩺 {lang === 'ko' ? '의료 정보' : 'Medical Info'}</div>
                <div style={{ background: 'var(--surface)', borderRadius: 14, padding: 16 }}>
                  {[
                    ...
                    { key: 'allergy', label: '알레르기' }, { key: 'address', label: '주소' },
                  ].map(f => (
